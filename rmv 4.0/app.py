...
def _same_origin_request(req):
    ...
    if origin:
        ...
        return True
    if referer:
        return referer.startswith(host+'/') or referer==host
    if sec_fetch_site in ('same-origin','same-site','none'):
        return True
    # Some browsers/clients omit Origin and Referer on same-site requests.
    return True
...
class FuncionarioArquivo(db.Model):
    ...
    ass_status=db.Column(db.String(20),default='nao_solicitada')   # nao_solicitada|pendente|assinado|expirado
    ass_token=db.Column(db.String(120))
    ass_expira_em=db.Column(db.DateTime)
    ass_codigo=db.Column(db.String(120))   # código público de validação
...
class PontoMarcacao(db.Model):
    ...
    tipo=db.Column(db.String(30),nullable=False,index=True)  # entrada|saida_intervalo|retorno_intervalo|saida
    data_hora=db.Column(db.DateTime,nullable=False,index=True,default=utcnow)
    origem=db.Column(db.String(20),default='web')  # web|admin|importacao
    ...
    def to_dict(self):
        ...
        d['data_ref']=self.data_hora.strftime('%Y-%m-%d') if self.data_hora else ''
        return d

class PontoAjuste(db.Model):
    id=db.Column(db.Integer,primary_key=True)
    funcionario_id=db.Column(db.Integer,db.ForeignKey('funcionario.id'),nullable=False,index=True)
    data_ref=db.Column(db.String(10),nullable=False,index=True)
    motivo=db.Column(db.Text,nullable=False)
    antes_json=db.Column(db.Text,default='[]')
    depois_json=db.Column(db.Text,default='[]')
    criado_por=db.Column(db.String(120))
    criado_em=db.Column(db.DateTime,default=utcnow)
    def to_dict(self):
        d={c.name:getattr(self,c.name) for c in self.__table__.columns}
...
class PontoFechamentoDia(db.Model):
    ...
    funcionario_id=db.Column(db.Integer,db.ForeignKey('funcionario.id'),nullable=False,index=True)
    data_ref=db.Column(db.String(10),nullable=False,index=True)
    status=db.Column(db.String(20),default='fechado')  # fechado|fechado_com_ressalvas
    observacao=db.Column(db.Text,default='')
    resumo_json=db.Column(db.Text,default='{}')
    fechado_por=db.Column(db.String(120))
    fechado_em=db.Column(db.DateTime,default=utcnow)
    __table_args__=(db.UniqueConstraint('funcionario_id','data_ref',name='uq_ponto_fechamento_func_data'),)
    def to_dict(self):
        d={c.name:getattr(self,c.name) for c in self.__table__.columns}
        d['fechado_em_fmt']=self.fechado_em.strftime('%d/%m/%Y %H:%M') if self.fechado_em else ''
        try:
...
@app.route('/api/medicoes/<int:id>/assinatura/solicitar',methods=['POST'])
@lr
def api_medicao_solicitar_assinatura(id):
    m=Medicao.query.get_or_404(id)
    if (m.status or '').lower()=='cancelada':
        return jsonify({'erro':'Nao e possivel solicitar assinatura para medicao cancelada.'}),400
    d=request.json or {}
    canal=(d.get('canal') or 'link').strip().lower()
    telefone=wa_norm_number((d.get('telefone') or '').strip())

    if not telefone and m.cliente_id:
        cli=Cliente.query.get(m.cliente_id)
        if cli and cli.telefone:
            telefone=wa_norm_number(cli.telefone)

    token=secrets.token_urlsafe(24)
    expira=utcnow()+timedelta(days=7)
    if not (m.assinatura_codigo or '').strip():
        m.assinatura_codigo=secrets.token_urlsafe(10)
    m.assinatura_token=token
    m.assinatura_status='pendente'
    m.assinatura_expira_em=expira
    db.session.commit()
    ...
    audit_event('medicao_assinatura_solicitada','usuario',session.get('uid'),'medicao',m.id,True,{
        'numero':m.numero,
